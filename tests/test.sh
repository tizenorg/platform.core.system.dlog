...
dlogctrl -p -t invalid_type                          &> /dev/null && echo "06 ERR" || echo "06 OK"  # invalid -t
dlogctrl -p -t common                                &> /dev/null && echo "07 OK"  || echo "07 ERR" # valid -t
dlogctrl -k some_test_key                            &> /dev/null && echo "08 ERR" || echo "08 OK"  # no -gs with -k
dlogctrl -k some_test_key -g                         &> /dev/null && echo "09 ERR" || echo "09 OK"  # non-existent key
dlogctrl -k some_test_key -s                         &> /dev/null && echo "10 ERR" || echo "10 OK"  # no value
dlogctrl -k some_test_key -s some_value              &> /dev/null && echo "11 OK"  || echo "11 ERR" # valid -s
dlogctrl -k some_test_key -g                         &> /dev/null && echo "12 OK"  || echo "12 ERR" # valid -g
dlogctrl -k some_test_key -g -s some_other_value     &> /dev/null && echo "13 ERR" || echo "13 OK"  # both -gs
dlogctrl -k some_test_key -g -c                      &> /dev/null && echo "14 ERR" || echo "14 OK"  # both -gc
dlogctrl -k some_test_key -c -s some_other_value     &> /dev/null && echo "15 ERR" || echo "15 OK"  # both -cs
dlogctrl -k some_test_key -g | grep some_value       &> /dev/null && echo "16 OK"  || echo "16 ERR" # checks whether the value is what we passed
dlogctrl -k some_test_key -g | grep some_other_value &> /dev/null && echo "17 ERR" || echo "17 OK"  # ditto
dlogctrl -k some_test_key -c                         &> /dev/null && echo "18 OK"  || echo "18 ERR" # clear the test key
dlogctrl -k some_test_key -c                         &> /dev/null && echo "19 ERR" || echo "19 OK"  # clear the now non-existent key
dlogctrl -k some_test_key -g                         &> /dev/null && echo "20 ERR" || echo "20 OK"  # get the now non-existent key
