#define LOG_TAG "SRPOL_LOGGER"

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#include <dlog.h>
#include <logcommon.h>

int main (int argc, char **argv)
{
	if (argc == 1) {
		printf ("\t- checking oversized buffer\n");
		{
			char huge_buffer [LOG_BUF_SIZE * 3];

			memset (huge_buffer, 'a', LOG_BUF_SIZE);
			memset (huge_buffer + LOG_BUF_SIZE, 'b', LOG_BUF_SIZE);
			memset (huge_buffer + LOG_BUF_SIZE * 2, 'c', LOG_BUF_SIZE);
			huge_buffer [LOG_BUF_SIZE * 3 - 1] = '\0';

			LOGE ("%s", huge_buffer);
		}

		printf ("\t- checking garbage data\n");
		{
			char buffer [LOG_BUF_SIZE];
			int i;

			for (i = 0; i < LOG_BUF_SIZE; ++i) {
				buffer [i] = rand() & 255;
			}

			LOGE ("%s", buffer);
		}
		return 0;
	} else {
		int mode = 0;
		int count = atoi(argv[1]);

		while (count--) {
			if (mode) // alternate logging levels to allow testing of filters
				LOGE("%s", "test data, level: error");
			else
				LOGI("%s", "test data, level: info");

			mode = !mode;
		}
	}

	return 0;
}

