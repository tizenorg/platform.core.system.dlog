...
void print_help ()
{
	printf ("dlogctrl - provides control over dlog configuration. Options:\n"
		"\t-t type   Specifies the config type to work with. Possible values are: common, kmsg (default is common)\n"
...
int main (int argc, char ** argv)
{
	...
	const char * filename = get_config_filename (CONFIG_COMMON);
	...
	for (;;) {
		...
		case 'p':
			...
			break;
		case 't':
			if (!strcmp (optarg, "kmsg"))
				filename = get_config_filename (CONFIG_KMSG);
			else if (!strcmp (optarg, "common"))
				filename = get_config_filename (CONFIG_COMMON);
			else {
				printf ("Unknown type! Possible values: common kmsg");
				exit (1);
			}
			break;
		case 'k':
	...
	if (!log_config_read (&conf, filename)) {
