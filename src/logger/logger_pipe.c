...
static int parse_command_line(const char* cmdl, struct logger* server, int wr_socket_fd)
{
	...
	reader->file.size = 0;
	reader->buf_id = LOG_ID_MAX;
	reader->dumpcount = 0;

	while ((option = getopt(argc, argv, "cgsdtf:r:n:v:b:")) != -1) {
		switch (option) {
		case 'c':
		case 'g':
		...
		case 'b':
			...
			if (reader->buf_id < 0 || reader->buf_id >= LOG_ID_MAX)
				break;
			break;
		case 'r':
			if (!isdigit(optarg[0]))
	...
	if ((reader->buf_id < LOG_ID_MAX) && (reader->buf_id >= 0)) {
		...
		if (reader->file.path != NULL) {
			...
			if (!pipe2(fds, O_CLOEXEC)) {
				...
				service_reader(server, reader);
			}
			free (reader);
		}
	} else {
		if (reader->file.path != NULL) free (reader->file.path);
		free (reader);
	}
...
static int service_writer_socket(struct logger* server,struct writer* wr, struct epoll_event* event)
{
	...
	do
	{
		...
		switch (msg->request) {
		case DLOG_REQ_PIPE :
			if (msg->length != sizeof(struct dlog_control_msg)) return EINVAL;
		...
		case DLOG_REQ_BUFFER_SIZE :
			if (msg->length != (sizeof(struct dlog_control_msg) + 1)) return EINVAL;

			buf_id = msg->data[0];
			if (buf_id < 0 || buf_id >= LOG_ID_MAX) return EINVAL;
		...
		case DLOG_REQ_CLEAR:
			...
			{
				struct reader* reader;
				struct log_buffer* lb = server->buffers[buf_id];
		...
		case DLOG_REQ_HANDLE_LOGUTIL:
...

			{
				...
				parse_command_line(msg->data, server, wr->socket_fd);
			...
			break;
		case DLOG_REQ_REMOVE_WRITER: // not actually used but no harm keeping it
			if (msg->length < (sizeof(struct dlog_control_msg))) return EINVAL;
			msg->data[msg->length - sizeof(struct dlog_control_msg) -1] = 0;

			{
				char * temp = strtok(msg->data, " ");
				buf_id = log_id_by_name(temp);
				if (buf_id < 0 || buf_id >= LOG_ID_MAX)
					return EINVAL;

				if (strlen(temp) == msg->length - sizeof(struct dlog_control_msg))
					temp = NULL;
				else
					temp = strtok(NULL, "\0");
...
static int service_writer_pipe(struct logger* server,struct writer* wr, struct epoll_event* event)
{
	...
	if (event->events & EPOLLIN)
	{
		...
		while (entry->len <= wr->readed && wr->readed)
		{
...
static struct logger* logger_create(char* path, struct log_config *conf)
{
	...
	for (i = 0; i < LOG_ID_MAX; i++) {
		sprintf(temp2, "%s_size", log_name_by_id (i));
		temp = log_config_get (conf, temp2);
		if (!temp) goto err4;
		size = atoi (temp);

		temp = log_config_get (conf, log_name_by_id (i));
		if (!temp) goto err4;

		if (!(l->buffers[i] = buffer_create(i, size, temp)))
			goto err4;
	}
	return l;
