...
const char * get_config_filename (config_type type)
{
	switch (type) {
	case CONFIG_KMSG:
		return getenv ("DLOG_CONFIG_KMSG_PATH") ?: "/run/dlog.conf";
	case CONFIG_COMMON:
	default:
		return getenv ("DLOG_CONFIG_PATH") ?: (TZ_SYS_ETC "/dlog.conf");
	}
}
...
int log_config_read (struct log_config* config, char const * filename)
{
	FILE * file;
